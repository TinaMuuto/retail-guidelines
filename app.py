...
# RENS: Fjernet usynligt U+00A0 i variablenavnet
...
def main():
    ...
    if st.button("Generér PPT", type="primary"):
        ...
        try:
            ...
            # NY OG FORBEDRET GRUPPERING
            groups_map: Dict[str, Dict[str, Any]] = {}
            for f in uploads or []:
                name, ext = os.path.splitext(f.name)
                lf = f.name.lower()
                
                # Fjern kendte tags for at finde den sande base (Setting Navnet)
                base_name = name
                
                is_line_drawing = any(k in lf for k in ["line","floorplan","drawing"])
                if is_line_drawing:
                    # Fjern line/floorplan/drawing fra navnet for at matche rendering/csv
                    base_name = re.sub(r"[\s_-]*(line|floorplan|drawing)$", "", base_name, flags=re.I).strip()
                
                # Den fulde base_name bruges som unik nøgle (f.eks. "Shop-the-look_2025_Q2 - dining 01")
                base_key = base_name.strip()
                
                # Sæt det menneskelige læsbare navn til Setting-titel (f.eks. "Dining 01")
